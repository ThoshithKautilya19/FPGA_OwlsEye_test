...
// /** Posit Function */
// // Function to multiply two FP16 Posit values and return the result
// inline void posit16_multiply(posit16_t a, posit16_t b, posit16_t &result) {
//     #pragma HLS PIPELINE  // Pipeline optimization for better performance
//     result = p16_mul(a, b);  // SoftPosit multiplication function
// }
// /** Posit Function end */
...
#pragma HLS ARRAY_PARTITION variable=acc_stream    complete dim=0
    ...
    mul_pixel_loop: for(unsigned int pixel_index=0; pixel_index <
            ...
            mul_loop: for(unsigned char fine_index=0; fine_index < fine; fine_index++) {
                ...
                posit16_t temp_result;
                conv_acc_t prev = ( acc_index == 0 ) ? conv_acc_t(0) : acc_cache[fine_index] ;

                // posit16_multiply(window_stream[fine_index].read(), weight_stream[fine_index].read(), temp_result);
                // acc_cache[fine_index] = prev + temp_result;
                posit16_t window_posit, weight_posit;
                window_posit = convertFloatToP16((float)window_stream[fine_index].read());
                weight_posit = convertFloatToP16((float)weight_stream[fine_index].read());
                posit16_multiply(window_posit, weight_posit, temp_result);
                acc_cache[fine_index] = prev + (conv_acc_t)convertP16ToDouble(temp_result);
